...
class DefaultCodeDefinitionSet implements CodeDefinitionSet
{
    ...
    public function __construct()
    {
        ...
        /* [img] image tag */
        $builder = new CodeDefinitionBuilder('img', '<div style="padding: 0px; overflow: hidden;" class="col-xs-24"><img class="img-full" src="{param}" /></div>');
        $builder->setUseOption(false)->setParseContent(false)->setBodyValidator($urlValidator);
        array_push($this->definitions, $builder->build());

        /* [img=alt text] image tag */
        $builder = new CodeDefinitionBuilder('img', '<div style="padding: 0px; overflow: hidden;" class="col-xs-24"><img class="img-full" style="padding: 0px;" src="{param}" alt="{option}" /></div>');
