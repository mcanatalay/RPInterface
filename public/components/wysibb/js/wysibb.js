...
//			toolbar:			false,
			//img upload config 
			imgupload:			true,
			img_uploadurl:		"http://www.rpinterface.com/upload/imgUpload",
			img_maxwidth:		800,
			img_maxheight:		800,
			hotkeys:			true,
			showHotkeys:		true,
...
//			direction:			"ltr",
			...
			buttons: 			"bold,italic,underline,strike,sup,sub,|,img,video,link,|,bullist,numlist,|,fontcolor,fontsize,fontfamily,|,justify,justifyleft,justifycenter,justifyright,|,quote,code,table,removeFormat",
			allButtons: {
				bold : {
					title: CURLANG.bold,
                                    ...
                                    modal: {
                                        ...
                                        tabs: [
                                            ...
                                            { //The second tab
                                                ...
                                                html: '<div id="imguploader"> <form id="fupform" class="upload" action="{img_uploadurl}" method="post" enctype="multipart/form-data" target="fupload"><input type="hidden" name="iframe" value="1"/><input type="hidden" name="idarea" value="' + id + '" /><div class="fileupload"><input id="fileupl" class="file" type="file" name="img" /> </div> </form> </div><iframe id="fupload" name="fupload" src="about:blank" frameborder="0" style="width:0px;height:0px;display:none"></iframe></div>'
                                            }
                                        ],
				...
				bullist : {
					...
					transform : {
						'<ul>{SELTEXT}</ul>':"[bullist]{SELTEXT}[/bullist]",
				...
				justify: {
					...
					buttonHTML: '<span class="fonticon ve-tlb-textleft1"><i class="fa fa-align-justify"></i></span>',
					groupkey: 'align',
					transform: {
		...
		init: function() {
			if (window.FormData != null) {
				...
				this.$block[0].ondrop = $.proxy(function(e) {
					...
					$.ajax({
						type: 'POST',
						url: "http://www.rpinterface.com/upload/imgUpload",
						data: fData,
						processData: false,
						contentType: false,
						...
						success: $.proxy(function(myData) {
                                                        var data = new Object();
                                                        data.status = 1;
                                                        data.image_link = myData;
                                                        data.thumb_link = myData;
