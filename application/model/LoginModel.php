...
class LoginModel{
    ...
    public static function saveTimestampOfLoginOfUser($user_name){
        $database = Database::getFactory()->getConnection();
        $sql = "UPDATE users SET user_last_login_timestamp = :user_last_login_timestamp, user_last_login_ip = :user_last_login_ip
                WHERE user_name = :user_name LIMIT 1";
        $sth = $database->prepare($sql);
        $sth->execute(array(':user_name' => $user_name, ':user_last_login_timestamp' => time(), ':user_last_login_ip' => $_SERVER['REMOTE_ADDR']));
    }
    ...
    public static function getFacebookLoginUrl(){
        $url = Config::get('URL') . Config::get('FACEBOOK_LOGIN_PATH');
        $scope = array('email');
    ...
    public static function loginWithFacebook(){
        ...
        if($facebook_user_data != null){
            $result = UserModel::getUserDataByFacebookUID($facebook_user_data['id']);
