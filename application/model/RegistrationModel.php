...
class RegistrationModel{
    ...
    public static function registerNewUser(){
        ...
        // if it is active send verification
        if(Config::get('EMAIL_VERIFICATION_ACTIVE')){
            //sends verification
            ...
            //verify itself
            if(self::skipVerification($user_id)){
                Session::add('feedback_positive', Text::get('FEEDBACK_ACCOUNT_SUCCESSFULLY_CREATED_WITHOUT_VERIFICATION'));
                return true;
            }
        }
    ...
    public static function validateUserName($user_name){
        ...
        // if username is too short (2), too long (18) or does not fit the pattern (aZ09)
        if (!preg_match('/^[a-zA-Z0-9]{2,18}$/', $user_name)){
    ...
    public static function writeNewUserToDatabase($user_name, $user_password_hash, $user_email, $user_creation_timestamp, $user_activation_hash){
        ...
        $query->execute(array(':user_name' => $user_name,
                              ...
                              ':user_role' => Config::get('DEFAULT_NEW_USER_ROLE'), //We are setting role of new user
        ...
        if ($count == 1) {
            // get user_id of the user that has been created, to keep things clean we SHOULDN'T use lastInsertId() here
            // $user_id = UserModel::getUserIdByUsername($user_name);
            $user_id = $database->lastInsertId();
            
            //Add subscription
            FeedModel::addSubscription($user_id, 1, $user_id);
            
            //Check if extra info included to system
            if(Config::get('USER_INFO_ACTIVE')){
                // write side data of user into database
    ...
    public static function rollbackRegistrationByUserId($user_id){
        ...
        $query->execute(array(':user_id' => $user_id));
        if(Config::get('USER_INFO_ACTIVE')){
            $query = $database->prepare("DELETE FROM user_info WHERE user_id = :user_id");
            $query->execute(array(':user_id' => $user_id)); 
        }
    }
    ...
    /**
     * if EMAIL_VERIFICATION_ACTIVE is set as false skips that process
     * 
     * @param int user_id user id
     * 
     * @return bool success status
     */
    public static function skipVerification($user_id){
        $database = Database::getFactory()->getConnection();
    ...
    public static function verifyNewUser($user_id, $user_activation_verification_code){
     ...
     public static function getFacebookRegisterUrl(){
        $url = Config::get('URL') . Config::get('FACEBOOK_REGISTER_PATH');
        $scope = array('email');

        return Social::getFacebookURL($url, $scope);
    }
    
    public static function connectToFacebook($user_id){
        $facebook_user_data = Social::getFacebookUser();
        
        if (UserModel::doesFacebookUserIDAlreadyExist($facebook_user_data['id'])){
            return false;
        }
        
        $database = Database::getFactory()->getConnection();
        $sql = "UPDATE users SET user_facebook_uid = :user_facebook_uid, user_facebook_active = :user_facebook_active
                 WHERE user_id = :user_id LIMIT 1";
        $query = $database->prepare($sql);
        $query->execute(array(':user_id' => $user_id, 'user_facebook_uid' => $facebook_user_data['id'], ':user_facebook_active' => 1));
    ...
    public static function disconnectFromFacebook($user_id){      
        $database = Database::getFactory()->getConnection();
        $sql = "UPDATE users SET user_facebook_uid = :user_facebook_uid, user_facebook_active = :user_facebook_active
                 WHERE user_id = :user_id LIMIT 1";
        $query = $database->prepare($sql);
        $query->execute(array(':user_id' => $user_id, 'user_facebook_uid' => null, ':user_facebook_active' => 0));
