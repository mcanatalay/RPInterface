...
class LoginController extends Controller{
    ...
    public function index(){
        if(Auth::checkLogged()) {
            ...
            $this->View->renderWithAll('login/index', $data);
        }
    }
    ...
    public function register(){
        if(LoginModel::isUserLoggedIn()){
            ...
            $this->View->renderWithAll('login/register');
        }
    }
    ...
    public function connectToFacebook(){
        if(Auth::checkLogged()){
            RegistrationModel::connectToFacebook(Session::get('user_id'));
            Redirect::to('profile');
        } else{
            Redirect::home();
        }
    ...
    public function disconnectFromFacebook(){
        if(Auth::checkLogged()){
            RegistrationModel::disconnectFromFacebook(Session::get('user_id'));
            Redirect::to('profile');
        } else{
            Redirect::home();
        }
     ...
     public function loginWithFacebook(){
        $login_successful = LoginModel::loginWithFacebook();
    ...
    public function requestPasswordReset(){
        $this->View->renderWithAll('login/requestPasswordReset');
    }
    ...
    public function verifyPasswordReset($user_name, $verification_code){
        ...
        if(PasswordResetModel::verifyPasswordReset($user_name, $verification_code)){
            ...
            $this->View->renderWithAll('login/verifyPasswordReset', array(
                'user_name' => $user_name,
     ...
     * POST request !
     */
    public function verifyPasswordReset_action(){
    ...
    public function client(){
        $sso = new codoforum_sso(array("client_id" => 'rpinterf',"secret" => 'UFOzS0L4lM#w',"timeout" => 6000));
        $account = array();
        if (USER_IS_LOGGED_IN) {
            ...
            $account['mail'] = $user->user_email; //Your logged in user's email id
        }
